...
export const useParallax = (stiffness = 250, damping = 20, force = 45) => {
  ...
  const headerPrimaryX = useTransform(x, (event) => event / force);
  const headerPrimaryY = useTransform(y, (event) => event / force);
  const headerSecondaryX = useTransform(x, (event) => event / -force);
  const headerSecondaryY = useTransform(y, (event) => event / -force);
